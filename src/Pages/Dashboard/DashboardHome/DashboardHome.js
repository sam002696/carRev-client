...
const DashboardHome = () => {
  ...
  // load orders
  useEffect(() => {
    let url = `https://carrev-server-production.up.railway.app/orders?email=${user.email}`;
    if (isAdmin) {
      url = `https://carrev-server-production.up.railway.app/orders`;
    }
    axios
      .get(url)
      .then((res) => {
  ...
  // load reviews
  useEffect(() => {
    const url = `https://carrev-server-production.up.railway.app/reviews`;
    ...
    <section id="dashboard_home" className="dashboard-home">
      ...
      <div>
        <div className="text-true-gray-800 font-medium space-y-2 mb-6">
          ...
          <div>
            <span className="font-my-title text-lg text-gray-500">
              User Type:{" "}
            </span>
            <span className="text-true-gray-800 font-medium">
