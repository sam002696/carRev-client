...
const ManageOrderItem = ({ order, orders, setOrders, index }) => {
  ...
  const onSubmit = (data) => {
    ...
    const url = `https://carrev-server-production.up.railway.app/orders/${order._id}`;
    axios
  ...
  const handleRemoveOrder = (id) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        const url = `https://carrev-server-production.up.railway.app/orders/${id}`;
        axios
          .delete(url)
          .then((res) => {
            console.log(res);
    ...
    >
      ...
      <div className="px-2 lg:px-4 pt-2 pb-4 md:py-4 md:flex-grow md:flex md:flex-nowrap">
        <div className="flex-auto font-my-title">
          ...
          <p>
            Customer:{" "}
            <span className="text-true-gray-800 font-medium">{order.name}</span>
          </p>
          <p>
            Ordered:{" "}
          ...
          <p>
            Status:{" "}
            <span className="text-true-gray-800 font-medium">
              {success ? newStatus : order?.status}
            </span>
          </p>
        </div>
        <div className="mt-3 md:mt-0 md:flex-shrink-0 text-center md:text-right">
          <form
            ...
            className="flex flex-nowrap my-2 md:mt-0 md:mb-3"
          >
            <select
            ...
            <input
              type="submit"
              value="Change"
              className="flex-shrink-0 btn-regular text-sm px-4 py-1"
            />
          </form>
          <button
