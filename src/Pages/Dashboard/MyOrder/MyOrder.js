    ...
    >
      <div className="flex-grow flex flex-nowrap">
        ...
        <div className="flex-grow py-1 px-2 font-my-title text-sm md:text-base">
          <h4 className="font-normal text-base tracking-normal md:text-lg">
            {name}
          </h4>
      ...
      <div className="flex-shrink-0 flex sm:flex-col items-center sm:items-end p-2 space-x-2 sm:space-x-0 sm:space-y-3">
        <button className="btn-regular w-20 py-1 px-4">Pay</button>
        <button
          onClick={() => handleRemoveOrder(order._id)}
          className={`btn-regular py-1 px-4 w-20 bg-red-600 ${
            status.toLowerCase() === "shipped" && "opacity-40"
          }`}
        >
          Cancel
        </button>
