  ...
  const handleRemoveProduct = (id) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        const url = `https://carrev-server-production.up.railway.app/products/${id}`;
        axios
          .delete(url)
          .then((res) => {
            console.log(res);
  ...
  return (
    ...
    >
      ...
      <div className="flex-grow flex flex-col justify-center p-2 font-my-title">
        <h4 className="font-normal text-sm tracking-normal md:text-lg">
          {name}
        </h4>
      ...
      <div className="flex-shrink-0 flex items-center">
        <button
          ...
          className={`m-1 md:m-2 ${
            (!isEnoughProducts || index < 10) && "opacity-50"
          }`}
